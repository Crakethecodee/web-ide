...
function Kt({ id: e, panelIds: t, storage: n }) {
	...
	if (i) try {
		...
		if (t) {
			...
			if (a && Array.isArray(a.layout) && t.length === a.layout.length) {
				const s = {};
				for (let l = 0; l < t.length; l++) s[t[l]] = a.layout[l];
				return s;
			}
		} else {
			...
			if (f.length === 1) {
				...
				if (a && Array.isArray(a.layout)) {
					...
					if (s.length === a.layout.length) {
						const l = {};
						for (let u = 0; u < s.length; u++) l[s[u]] = a.layout[u];
						return l;
					}
				}
			}
...
function on({ debounceSaveMs: e = 100, panelIds: t, storage: n = localStorage, ...o }) {
	const i = t !== void 0, r = "id" in o ? o.id : o.groupId, f = he(r, t ?? []), a = (0, import_react.useSyncExternalStore)(Xt, () => n.getItem(f), () => n.getItem(f)), s = (0, import_react.useMemo)(() => {
		if (a) {
			const c = JSON.parse(a), p = Object.values(c);
			if (Array.from(p).every((m) => typeof m == "number")) return c;
		}
	}, [a]), l = (0, import_react.useMemo)(() => {
		if (!s) return Kt({
			id: r,
			panelIds: t,
			storage: n
		...
		t,
		n
	]), u = s ?? l, h = (0, import_react.useRef)(null), d = (0, import_react.useCallback)(() => {
		const c = h.current;
	...
	const S = (0, import_react.useCallback)((c) => {
		d();
		let p;
	...
	return {
		...
		*/
		onLayoutChange: (0, import_react.useCallback)((c) => {
...
//#endregion
export { Ut as Group, Yt as Panel, Qt as Separator, wt as isCoarsePointer, on as useDefaultLayout, rn as useGroupCallbackRef, sn as useGroupRef, an as usePanelCallbackRef, ln as usePanelRef };
